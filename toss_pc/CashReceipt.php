<?php
	...
	$LGD_METHOD   		    	= $_POST["LGD_METHOD"];							//메소드('AUTH':승인, 'CANCEL' 취소)
    $LGD_OID                	= $_POST["LGD_OID"];							//주문번호(상점정의 유니크한 주문번호를 입력하세요)
    $LGD_PAYTYPE                = $_POST["LGD_PAYTYPE"];						//결제수단 코드 (SC0030:계좌이체, SC0040:가상계좌, SC0100:무통장입금 단독)
    $LGD_AMOUNT     		    = $_POST["LGD_AMOUNT"];            				//금액("," 를 제외한 금액을 입력하세요)
    $LGD_CASHCARDNUM        	= $_POST["LGD_CASHCARDNUM"];					//발급번호(주민등록번호,현금영수증카드번호,휴대폰번호 등등)
    $LGD_CUSTOM_MERTNAME 		= $_POST["LGD_CUSTOM_MERTNAME"];    			//상점명
    $LGD_CUSTOM_BUSINESSNUM 	= $_POST["LGD_CUSTOM_BUSINESSNUM"];				//사업자등록번호
    $LGD_CUSTOM_MERTPHONE 		= $_POST["LGD_CUSTOM_MERTPHONE"];    			//상점 전화번호
    $LGD_CASHRECEIPTUSE     	= $_POST["LGD_CASHRECEIPTUSE"];					//현금영수증발급용도('1':소득공제, '2':지출증빙)
